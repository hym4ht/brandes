...
@push('styles')
    @vite([
        'resources/css/user/log-aktivitas.css',
                        ...
                        <tr class="data-row {{ $loop->last ? 'last-row' : '' }}" data-id="{{ $log->id }}">
                            ...
                            <td>
                                @if($log->file_ktp)
                                    @if(isset($log->status_ktp) && $log->status_ktp == 'Diambil')
                                        <span class="badge-status nonaktif" style="cursor: pointer;" onclick="openDetailDokumenModal('KTP', '{{ $log->file_ktp }}', '{{ route('dokumen.download', $log->file_ktp) }}')">KTP</span>
                                    @else
                                        <span class="badge-status aktif" style="cursor: pointer;" onclick="openDetailDokumenModal('KTP', '{{ $log->file_ktp }}', '{{ route('dokumen.download', $log->file_ktp) }}')">KTP</span>
                                    @endif
                                @else
                            ...
                            <td>
                                @if($log->file_kk)
                                    @if(isset($log->status_kk) && $log->status_kk == 'Diambil')
                                        <span class="badge-status nonaktif" style="cursor: pointer;" onclick="openDetailDokumenModal('KK', '{{ $log->file_kk }}', '{{ route('dokumen.download', $log->file_kk) }}')">KK</span>
                                    @else
                                        <span class="badge-status aktif" style="cursor: pointer;" onclick="openDetailDokumenModal('KK', '{{ $log->file_kk }}', '{{ route('dokumen.download', $log->file_kk) }}')">KK</span>
                                    @endif
                                @else
                                    <span class="badge-status nonaktif">Belum Ada</span>
                                @endif
                            ...
                            <td>
                                @if($log->file_akte)
                                    @if(isset($log->status_akte) && $log->status_akte == 'Diambil')
                                        <span class="badge-status nonaktif" style="cursor: pointer;" onclick="openDetailDokumenModal('AKTE', '{{ $log->file_akte }}', '{{ route('dokumen.download', $log->file_akte) }}')">AKTE</span>
                                    @else
                                        <span class="badge-status aktif" style="cursor: pointer;" onclick="openDetailDokumenModal('AKTE', '{{ $log->file_akte }}', '{{ route('dokumen.download', $log->file_akte) }}')">AKTE</span>
                                    @endif
                                @else
                                    <span class="badge-status nonaktif">Belum Ada</span>
                                @endif
                            ...
                            <td>
                                <x-c-shared.opsi :item="$log" type="log-user">
                                    <form action="{{ route('user.log.aktivitas.ambil', $log->id) }}" method="POST" style="margin: 0; padding: 0;">
                                        @csrf
...
@push('scripts')
    @vite([
        ...
        'resources/js/user/log-aktivitas.js'
    ])
