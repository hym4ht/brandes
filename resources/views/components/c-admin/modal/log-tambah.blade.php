    ...
    <div class="modal-container log-modal-container" style="position: relative;">
        @if(in_array(session('user.role'), ['superadmin', 'admin']))
            <x-c-shared.tooltip-info 
                ...
                position="left" 
                target=".modal-container.log-modal-container"
            />
        @endif
        ...
        <div class="modal-body">
            <form action="{{ route('log.berkas.store') }}" method="POST" id="formTambahLog" enctype="multipart/form-data">
                @csrf
            ...
            <div class="tooltip-target-wrapper" style="width: 100%;">
                ...
                @if(in_array(session('user.role'), ['superadmin', 'admin']))
                    <x-c-shared.tooltip-info 
                        number="4" 
                        title="info 4" 
                        text="jika sudah melakukan pengisian data, selanjutnya klik button &quot;simpan data&quot;" 
                        position="right" 
                        target="#btnSubmitLog"
                    />
                @endif
